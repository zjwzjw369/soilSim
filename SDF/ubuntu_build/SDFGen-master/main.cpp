...
int main(int argc, char* argv[]) {
  ...
  std::string outname;
	...
	if(phi_grid.a[i] >= -0.05 && phi_grid.a[i] <= 0.05)
	   distance->SetValue(i, 1);
        else
	   distance->SetValue(i,0);
    }
	...
	std::string outname1 = "/home/zhanghaitong/Desktop/SDFGen-master/models/1.txt";
    std::ofstream outfile1( outname1.c_str());
outfile1 <<phi_grid.ni << std::endl;
outfile1 <<phi_grid.nj << std::endl;
outfile1 <<phi_grid.nk << std::endl;
    for(unsigned int i = 0; i < phi_grid.a.size(); ++i) {
      outfile1 << phi_grid.a[i] << std::endl;
    }
    outfile1.close();
  #else
